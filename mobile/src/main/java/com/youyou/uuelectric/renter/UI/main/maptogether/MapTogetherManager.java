...
public class MapTogetherManager {
    ...
    public static MapTogetherManager getInstance(MainMapFragment mainMapFragment, Activity mContext) {
        if (instance == null) {
            instance = new MapTogetherManager(mainMapFragment, mContext);
        }
        return instance;
    }

    private MapTogetherManager(MainMapFragment mainMapFragment, Activity mContext) {
     ...
     */
    public void onMapLoadedUpdateMarkerText(final List<DotInterface.DotInfo> allDotsList, final Map<String, Marker> markerMap) {
        // 循环遍历更新聚合marker显示文案
        if (togDotInfoMap != null && togDotInfoMap.entrySet().size() > 0) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    L.i("开始循环遍历,执行更新网点text内容的操作...");
                    ...
                    while (entryIterator.hasNext()) {
                        ...
                        if (listMarker != null && listMarker.size() > 0) {
                            ...
                            for (Marker marker : listMarker) {
                                ...
                                count = count + ((DotInterface.DotInfo)marker.getObject()).getCarTotal();
                            }
    ...
    private void assignSingleCluster(Marker marker) {
        ...
        Point point = mainMapFragment.aMap.getProjection().toScreenLocation(latLng);
    ...
    private void addTogDotInfoToMap(String dotId, TogDotInfo togDotInfo) {
        ...
        Marker marker = mainMapFragment.aMap.addMarker(markerOptions);
